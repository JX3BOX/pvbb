    ...
    watch: {
        ...
        autoResize: {
            handler(newV) {
                if (!newV) return;
                setTimeout(() => {
                    const frame = this.$refs.autoExpand;
    ...
    methods: {
        ...
        initItem(start = 0) {
            ...
            list.forEach((e, i) => {
                ...
                const host = this.getColDom(_i);
                const imgDom =
                    host && host.getElementsByClassName ? host.getElementsByClassName("waterfall-img")[0] : null;
                this.loader(
                    e[this.imgKey],
                    () => {
                        loadNum++;
                        ...
                        // Vue3: 直接赋值/替换即可触发响应
                        this.styleArr.splice(_i, 1, { ...this.styleArr[_i] });
        ...
        async calcXY(index = 0, cause = "data", duration) {
            ...
            for (let i = index; i < this.styleArr.length; i++) {
                ...
                if (this.styleArr[i].showClass) {
                    this.styleArr[i]["transition-duration"] = `${moveDuration}s`;
                }
            }
            await this.$nextTick();
